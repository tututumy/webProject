...
export default class compre_relateMsg extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visibleImg: false,
        ...
        if (res && res.data) {
          ...
          for (let i = 0; i < res.data[0].length; i++) {
            if (
              ...
              res.data[0][i].fileSource ===language[`superior_${this.props.language.getlanguages}`]
            ) {
              source = language[`superior_${this.props.language.getlanguages}`];
            } else if (
              res.data[0][i].fileSource === "本级" ||
              res.data[0][i].fileSource ==language[`level_${this.props.language.getlanguages}`]
            ) {
    ...
    if (type === "图片") {
      ...
      if (data.length > 0) {
        ...
        if (this.props.All.imgData_Special) {
          let data_model = this.props.All.imgData_Special;
          dataSource = this.props.All.imgData_Special;
        ...
        //把图片的编号发给后端更新缓存
        this.props.dispatch({
          type: "All/updateImgData_Special",
          payload: {
            dataSourceFileId: dataSourceFileId
          }
        });
        this.props.dispatch({
          type: "All/insertImgData_Special",
          payload: {
      ...
      if (data.length > 0) {
        ...
        if (this.props.All.videoData_Special) {
          let data_model = this.props.All.videoData_Special;
          dataSource = this.props.All.videoData_Special;
          for (let i = 0; i < data_model.length; i++) {
            arr.unshift(data_model[i].fileId); //arr存放了已经存在的图片列表中的图片名称
            dataSourceFileId.unshift({ fileId: data_model[i].fileId });
          }
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          //把图片的编号发给后端更新缓存
          this.props.dispatch({
            type: "All/updateVideoData_Special",
            payload: {
              dataSourceFileId: dataSourceFileId
            }
          });
          this.props.dispatch({
            type: "All/insertVideoData_Special",
            payload: {
              dataSource: dataSource
            }
          });
      ...
      if (data.length > 0) {
        ...
        if (this.props.All.htmlData_Special) {
          // 查询出来的图片素材信息
          let data_model = this.props.All.htmlData_Special;
          dataSource = this.props.All.htmlData_Special; //将查询出来的素材信息赋值到dataSource
          for (let i = 0; i < data_model.length; i++) {
        ...
        //把图片的编号发给后端更新缓存
        this.props.dispatch({
          type: "All/updateHtmlData_Special",
          payload: {
            dataSourceFileId: dataSourceFileId
          }
        });
        this.props.dispatch({
          type: "All/insertHtmlData_Special",
          payload: {
            dataSource: dataSource
          }
        });
      ...
      if (data.length > 0) {
        ...
        if (this.props.All.docData_Special) {
          let data_model = this.props.All.docData_Special;
          dataSource = this.props.All.docData_Special;
          for (let i = 0; i < data_model.length; i++) {
            arr.unshift(data_model[i].fileId); //arr存放了已经存在的图片列表中的图片名称
            dataSourceFileId.unshift({ fileId: data_model[i].fileId });
          }
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          //把图片的编号发给后端更新缓存
          this.props.dispatch({
            type: "All/updateDocData_Special",
            payload: {
              dataSourceFileId: dataSourceFileId
            }
          });
          this.props.dispatch({
            type: "All/insertDocData_Special",
            payload: {
              dataSource: dataSource
            }
          });
          arr.push(data[i].fileId);
        }
      }
    }
  };
  ...
  handleCancelLookVideo = () => {
    ...
    document.getElementById("my_video1").pause();
  };
  ...
  //删除图片素材
  handleClickDelete = (fileId, fileName, type) => {
    if (type === "图片") {
      const dataSource = [...this.props.All.imgData_Special];
      let data = dataSource.filter(item => item.fileId !== fileId);
      this.props.dispatch({
        type: "All/deleteImgColumndata",
        payload: data
      });
    } else if (type === "视频") {
      const dataSource = [...this.props.All.videoData_Special];
      let data = dataSource.filter(item => item.fileId !== fileId);
      this.props.dispatch({
        type: "All/deleteVideoColumndata",
        payload: data
      });
    } else if (type === "网页") {
      const dataSource = [...this.props.All.htmlData_Special];
      let data = dataSource.filter(item => item.fileId !== fileId);
      this.props.dispatch({
        type: "All/deleteHtmlColumndata",
        payload: data
      });
    } else if (type === "文档") {
      const dataSource = [...this.props.All.docData_Special];
      let data = dataSource.filter(item => item.fileId !== fileId);
      this.props.dispatch({
        type: "All/deleteDocColumndata",
        payload: data
      });
    }
  };
  ...
  handleLookMsg = (num, name, type) => {
    ...
    if (type === "图片") {
      ...
      axios({
        ...
        .then(res => {
          if (res && res.data) {
            document.getElementById("showImg1").src = routerMsg + res.data[0];
          }
        })
        .catch(error => {
          error.response  && error.response.status? responseStatus(error.response.status) : message.error(`error: ${error}`)
      ...
      axios({
        ...
        .then(res => {
          if (res && res.data) {
            document.getElementById("my_video1").src = routerMsg + res.data[0];
  ...
  render() {
        ...
        render: (text, record) => (
          <div>
            ...
            <Popconfirm
              ...
              onConfirm={() =>
                this.handleClickDelete(record.fileId, record.fileName, "图片")
              }
            >
        ...
        render: (text, record) => (
          <div>
            ...
            <Popconfirm
              ...
              className={styleless.popConfirm}
              onConfirm={() =>
                this.handleClickDelete(record.fileId, record.fileName, "视频")
              }
            >
              <a>{language[`delete_${this.props.language.getlanguages}`]}</a>
            </Popconfirm>
        ...
        render: (text, record) => (
          <div>
            ...
            <Popconfirm
              ...
              className={styleless.popConfirm}
              onConfirm={() =>
                this.handleClickDelete(record.fileId, record.fileName, "网页")
              }
            >
              <a>{language[`delete_${this.props.language.getlanguages}`]}</a>
            </Popconfirm>
        ...
        render: (text, record) => (
          <div>
            ...
            <Popconfirm
              ...
              className={styleless.popConfirm}
              onConfirm={() =>
                this.handleClickDelete(record.fileId, record.fileName, "文档")
              }
            >
              <a>{language[`delete_${this.props.language.getlanguages}`]}</a>
            </Popconfirm>
    ...
    let imgTabledata = [];
    let fileSourceImg;
    let imgTabledataCache = this.props.All.imgData_Special
      ? this.props.All.imgData_Special
      : [];
    ...
    let videoTabledata = [];
    let fileSourceVideo;
    let videoTabledataCache = this.props.All.videoData_Special
      ? this.props.All.videoData_Special
      : [];
    ...
    let htmlTabledata = [];
    let fileSourceHtml;
    let htmlTabledataCache = this.props.All.htmlData_Special
      ? this.props.All.htmlData_Special
      : [];
    ...
    let docTabledata = [];
    let fileSourceDoc;
    let docTabledataCache = this.props.All.docData_Special
      ? this.props.All.docData_Special
      : [];
