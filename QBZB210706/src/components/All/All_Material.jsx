...
let showMsg = false;
@connect(({ fodder, table, language, loading }) => ({
  fodder,
  table,
  language,
  loading
...
class FodderContent extends Component {
    ...
    if (type === language[`webPage_${this.props.language.getlanguages}`]) {
      ...
      axios({
        ...
        .then(res => {
          ...
          // let newWD = window.open(routerMsg + res.data[0], "_blank");
          // newWD.document.title = name;
          // let iframe = newWD.document.createElement("IFRAME");
          // iframe.setAttribute('src', url);
          // iframe.setAttribute('style', 'position:absolute;top:0;left:0;width:100%;height:100%;');
          // newWD.document.body.appendChild(iframe);

          let newWD = window.open()
          newWD.document.write(`<html><title>${name}<\/title>`)
          newWD.document.write("<head>")
          newWD.document.write("<\/head>")
          newWD.document.write("<body style='margin:0'>")
          newWD.document.write("<embed width='100%' height='100%' src="+routerMsg + res.data[0]+"> </embed>")
          newWD.document.write("<\/body>")
          newWD.document.write("<\/html>")
        })
        .catch(error => {
          error.response  && error.response.status? responseStatus(error.response.status) : message.error(`error: ${error}`)
        });
      ...
      type ===
      language[`ReconnaissanceSignalData_${this.props.language.getlanguages}`]
    ) {
      let urll;
      if (typeof window.getUrl == "function") {
        urll = window.getUrl() + "/api/LK-0313036/LK036";
      ...
      axios({
        ...
        .then(this.context.router.history.push("/radarinformation?id=view"))
        .catch(error => {
          error.response  && error.response.status? responseStatus(error.response.status) : message.error(`error: ${error}`)
        });
    }
  ...
  beforeUpload = (file, fileList) => {
    ...
    if (!isLt100M) {
      message.error(file.name +language[`Picture_size_out_of_limit_${this.props.language.getlanguages}`]);
      return isLt100M;
    }
  ...
  handleChange = info => {
    ...
    if (info.file.status === "done") {
      ...
      message.success(`${info.file.name.slice(0, 15) + "..."}` +
  ...
  handleClickDelete = fileId => {
    this.props.dispatch({
      ...
      callback: res => {
        if (res.data == "0") {
          message.warning(language[`materialUsedNoDelete_${this.props.language.getlanguages}`]);
        } else {
          message.success(language[`MaterialDeletedSuccessfully_${this.props.language.getlanguages}`]);
  ...
  deleteAll = () => {
    if (this.state.selectedRowKeys.length === 0) {
      message.warning(language[`PleaseSelectTheMaterialFirst_${this.props.language.getlanguages}`]);
    } else {
      this.setState({
        visibleConfirm: true
      });
  ...
  handleOkConfirm = () => {
    ...
    // console.log("3333333333",this.state.selectedRows)
    // if(this.state.selectedRows&&this.state.selectedRows.length>100){
    //   message.warning("最多只能同时删除100条数据，请重新选择！")
    //   this.setState({visibleConfirm:false,selectedRows:"",selectedRowKeys: []})
    //   return false;
    // }
    this.props.dispatch({
      ...
      payload: {
        ...
        // allFileId: this.state.selectedRows.join(",")
      },
      callback: res => {
        if (res.data == "0") {
          message.warning(
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: language[`MaterialType_${this.props.language.getlanguages}`],
        dataIndex: "type"
      },
      {
        title:language[`MaterialGenerationTime_${this.props.language.getlanguages}`],
        dataIndex: "datatime",
        width: "20%"
      },
      {
        title: language[`InformationsViewer_${this.props.language.getlanguages}`],
        dataIndex: "",
        // key: 'num',
        render: (text, record) => (
          //为了假数据注释掉的
          // this.state.getdata.length >= 1
          //   ? (
          //     <Link to="/radarinformation?id=view"><span onClick={() => this.handleClickLook(record.num, record.type)} >查看</span></Link>
          //   ) : null
          // <Link to="/radarinformation?id=view"></Link>
          <a
            onClick={() =>
      ...
      <Fragment>
        ...
        <div className={style.Content}>
            ...
            >
              <div className={style.popFodderType}>
                ...
                <div className={style.title_wrapper}>
                  ...
                  {/* 素材类型 */}
                  {/* 中文样式 */}
                  {/* <div className={style.popselect} style={{marginLeft: '20px'}}> */}
                  {/* 法文样式 */}
                  <div className={style.popselect}>
                    <span style={{ marginRight: "10px" }}>
          ...
          <div className={style.TableContent_material}>
            <Table
              loading={this.props.loading.effects["fodder/selectAllMaterical"]}
              bordered
          ...
          {this.state.LookImg ? (
            <Dialog
              BodyContent={<div className={style.popFodderTypeImg}>
                  {/*上传后的文件列表*/}
          ...
          {this.state.LookVideo ? (
            <Dialog
              BodyContent={<div className={style.popFodderType}>
                  {/*上传后的文件列表*/}
                  <div className={style.uploadtable}>
                    <div
                      style={{        width: "600px",
...
class SideBar extends Component {
  ...
  handleChange = e => {
    if (e.target.name === "typeCheck1") {
      this.setState({ typeCheck1: !this.state.typeCheck1 }, function () {
        this.props.dispatch({
          ...
          payload: {
            ...
            h: this.state.unitCheck3 ? 1 : 0
          }
        });
      });
    } else if (e.target.name === "typeCheck2") {
      this.setState({ typeCheck2: !this.state.typeCheck2 }, function () {
        this.props.dispatch({
          type: "fodder/ClickBtn",
          payload: {
            a: this.state.typeCheck1 ? 1 : 0,
      ...
      this.setState({ typeCheck3: !this.state.typeCheck3 }, function () {
        this.props.dispatch({
          type: "fodder/ClickBtn",
          payload: {
            a: this.state.typeCheck1 ? 1 : 0,
    ...
    } else if (e.target.name === "typeCheck4") {
      this.setState({ typeCheck4: !this.state.typeCheck4 }, function () {
        this.props.dispatch({
          type: "fodder/ClickBtn",
          payload: {
            a: this.state.typeCheck1 ? 1 : 0,
    ...
    } else if (e.target.name === "typeCheck5") {
      this.setState({ typeCheck5: !this.state.typeCheck5 }, function () {
        this.props.dispatch({
          type: "fodder/ClickBtn",
          payload: {
            a: this.state.typeCheck1 ? 1 : 0,
      ...
      this.setState({ unitCheck1: !this.state.unitCheck1 }, function () {
        this.props.dispatch({
          type: "fodder/ClickBtn",
          payload: {
            a: this.state.typeCheck1 ? 1 : 0,
      ...
      this.setState({ unitCheck2: !this.state.unitCheck2 }, function () {
        this.props.dispatch({
          type: "fodder/ClickBtn",
          payload: {
            a: this.state.typeCheck1 ? 1 : 0,
    ...
    } else if (e.target.name === "unitCheck3") {
      this.setState({ unitCheck3: !this.state.unitCheck3 }, function () {
        this.props.dispatch({
          type: "fodder/ClickBtn",
          payload: {
            a: this.state.typeCheck1 ? 1 : 0,
