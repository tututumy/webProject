...
let showMsg = false;
@connect(({ fodder, table, language, ElectronicTarget, loading }) => ({
  fodder,
  table,
  language,
  ElectronicTarget,
...
class FodderContent extends Component {
  ...
  beforeUpload = (file, fileList) => {
    ...
    if (fileFormatCount == 0) {
      message.warning(
        `${file.name}${
          language[`typeMismatch_${this.props.language.getlanguages}`]
        }`
      );
      return false;
    }
    ...
    for (let i = 0; i < data.length; i++) {
      if (file.name == data[i].name) {
        message.warning(
          `${file.name}${
            language[
              `Uploaded_cannot_upload_repeatedly_${
    ...
    if (!isLt100M) {
      message.error(
        file.name +language[`Picture_size_out_of_limit_${this.props.language.getlanguages}`
          ]
      );
      return isLt100M;
    }
  ...
  handleChange = info => {
    ...
    if (info.file.status === "uploading") {
      ...
      document.getElementById("makeSureUpload1").disabled = "disabled";
    } else {
      this.setState({ uploadLoadingMark: false });
      document.getElementById("makeSureUpload1").disabled = false;
    }
                ...
                <div className={style.title_wrapper}>
                  ...
                  <div className={style.popselect}>
                    ...
                    >
                      ...
                      {/* <Option value="侦察信号数据">{language[`ReconnaissanceSignalData_${this.props.language.getlanguages}`]}</Option> */}
                    </Select>
                  </div>
                  {/* 提供上传*/}
